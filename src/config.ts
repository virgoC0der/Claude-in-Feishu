...
export function loadConfig(): Config {
  ...
  return {
    ...
    discordTtsEnabled: env.get("CTI_DISCORD_TTS_ENABLED") === "true",
    fishAudioApiKey: env.get("CTI_FISH_AUDIO_API_KEY") || undefined,
    fishAudioVoiceId: env.get("CTI_FISH_AUDIO_VOICE_ID") || undefined,
  };
}
...
export function configToSettings(config: Config): Map<string, string> {
  ...
  m.set("bridge_discord_tts_enabled", config.discordTtsEnabled ? "true" : "false");
  if (config.fishAudioApiKey) m.set("bridge_discord_fish_audio_api_key", config.fishAudioApiKey);
  if (config.fishAudioVoiceId) m.set("bridge_discord_fish_audio_voice_id", config.fishAudioVoiceId);
